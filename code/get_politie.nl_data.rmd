...
Code Chunk 2: Downloads table of contents and catalog data from politie.nl to acquire the necessary metadata via the CBS API.
```{r metadata-load}
toc <- cbs_get_toc()
...
Code Chunk 3: Creates 6 separate vectors to be used in the data collection process, with the first including time parameters, while the other 5 each contain geometrical parameters.
```{r vector-parameters}
...
Code Chunk 4: Downloads data in 5 parts, as per the previously defined vectors
```{r data-download}
...
Code Chunk 5: Combines the 5 separately downloaded datasets into one.
```{r data-bind}
all_data <- rbind(data_1, data_2, data_3, data_4, data_5)
```

for(i in 1:nrow(all_data)) {       # for-loop over rows
  all_data[i, ] <- all_data[i, ] {
  if all_data["0.0.0"]:
    all_data["0.0.0"] <- "Total"}
  if all_data != all_data["0.0.0"]:
      break
}
...
for(i in 1:nrow(all_data)){
  if(all_data == all_data$SoortMisdrijf[0.0.0]{
    all_data[0.0.0] <- all_data["Total"]
  }
  if(all_data != 0.0.0){
    break
  }
...
for(i in 1:nrow(all_data)){
  if(all_data$SoortMisdrijf[i] == 0.0.0){
    all_data$SoortMisdrijf["0.0.0"] <- all_data$SoortMisdrijf["Total"]
    break
  }
}
...
all_data %>% 
  select(SoortMisdrijf(0.0.0)) %>% 
  rename_with(str(Total)
...
names(all_data)[names(all_data) == "BU03630000"] <- "AMS"
View(all_data)
BU03630000
...
for(i in 1:nrow(all_data)){
  if(all_data$WijkenEnBuurten[i] == as.factor(BU03630000)){
    all_data$WijkenEnBuurten["BU03630000"] <- all_data$WijkenEnBuurten["AMS"]
    break
  }
}
View(all_data)
...
all_data2 <- FindReplace(data = all_data, Var = "0.0.0", replaceData =
                       from = "0.0.0", to = "Total", exact = FALSE)



all_data <- data.frame(from = as.character(0.0.0), to = "Totaal")


all_data$SoortMisdrijf[all_data$SoortMisdrijf == "0.0.0"] <- "Total"
View(all_data)
