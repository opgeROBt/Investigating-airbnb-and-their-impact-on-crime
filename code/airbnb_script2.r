...
#Check if we can use the lapply to write it to a file
#note Rob:just couldn't get the lapply to work! I created the forloop that creates the dataset above
#it would run but give an error in the end (maybe needed to give an df before running it)
lapply(all_data,  for (val in WijkEnBuurten_combined) api_call_cbsdata(val), USE.NAMES = TRUE)
...
#remapping the crimetypes 
#edit Rob: later it might be possible to create a dataframe with a column of crimetype names, and crimetype codes and we would be able to use this for more comnpact code
...
for (.v in gra) all_data$CrimeType <- str_replace(WijkEnBuurten_combined$CrimeType, .v, "Other")


#remapping the city areas to readable names in the all_data dataset 
#important! Take note that the full dataset has been put in the 'all_data' dataset. for the code below it needs to be changed form wijkenbuurten_combined to all_data 
#important2:currently the city area for loops are writen to WijkEnBuurten_combined$CrimeType  this means it replaces values in the crimetype collumn, we might wanna change that to the $neighborhoods column 

#check the attributes function to see what is included in the dataset (more then is visible from the column)
attr(all_data$CrimeType, "labels") 
attr(all_data$Periods, "labels") 
attr(all_data$Periods, "names") 


#note: probably this can be automated a bit more. 
...
for (.v in centrum_west) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Centrum-West")
...
for (.v in de_akker_nieuw_sloten) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "De Akker - Nieuw Sloten")
...
for (.v in de_pijp_rivierenbuurt) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "De Pijp - Rivierenbuurt")
...
for (.v in gaasperdam_driemond) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Gaasperdam - Driemond")
...
for (.v in geuzenveld_slotermeer) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Geuzenveld - Slotermeer")
...
for (.v in ijburg_zeeburgereiland) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "IJburg - Zeeburgereiland")
...
for (.v in noord_oost) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Noord-Oost")
...
for (.v in noord_west) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Noord-West")
...
for (.v in oostelijk_havengebied_indische_buurt) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Oostelijk Havengebied - Indische Buurt")
...
for (.v in osdorp) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Osdorp")
...
for (.v in de_baarsjes_oud_west) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "De Baarsjes - Oud-West")
...
for (.v in oud_noord) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Oud-Noord")
...
for (.v in oud_oost) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Oud-Oost")
...
for (.v in zuid) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Zuid")
...
for (.v in slotervaart) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Slotervaart")
...
for (.v in watergraafsmeer) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Watergraafsmeer")
...
for (.v in westerpark) all_data$CrimeType <- str_replace(all_data$CrimeType, .v, "Westerpark")

# Since it is not possible to create the next chucnk of code in a for loop. We did hard-code it.
#note Rob: not sure if this is needed since it is still in the attributes (labels) 
...
#this part below seems to be in markdown
