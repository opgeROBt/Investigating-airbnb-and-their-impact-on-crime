...
# Run the  code for every value in wijkEnBuurten_combined and write it to pls.datatable
pls.datatable <- data.frame() 
for (val in WijkEnBuurten_combined) 
{API_data_neighbourhood <-  api_call_cbsdata(val)
pls.datatable <- rbind(pls.datatable, API_data_neighbourhood)
}
'

# Renaming the columns to English
names(pls.datatable)[names(pls.datatable) == "WijkenEnBuurten"] <- "Neighborhoods"
names(pls.datatable)[names(pls.datatable) == "Perioden"] <- "Periods"
names(pls.datatable)[names(pls.datatable) == "SoortMisdrijf"] <- "CrimeType"
names(pls.datatable)[names(pls.datatable) == "GeregistreerdeMisdrijven_1"] <- "RegisteredCrimes"

# Categorizing the different crimes underneath each CrimeType. 
pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, "0.0.0", "Total")

# Yellow = Robbery/Theft
rob <- c("1.1.1", "1.1.2", "1.2.1", "1.2.2", "1.2.3", "1.2.4", "1.2.5", "1.4.6", "1.4.7", "1.5.2", "1.6.2", "2.5.1", "2.5.2")
for (.v in rob)  pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Robbery/Theft")

# Red = Violence
vio <- c("1.3.1", "1.4.1", "1.4.2", "1.4.3", "1.4.4", "1.4.5", "1.6.1", "2.2.1", "2.4.1", "2.4.2", "3.6.4")
for (.v in vio) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Violence")

# Purple = Smuggling
smu <- c("1.6.3", "3.1.1", "3.1.2", "3.1.3", "3.2.1", "3.2.2")
for (.v in smu) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Smuggling")

# Blue = Inappropriateness
ina <- c("2.1.1","3.3.2","3.3.5","3.4.2","3.5.2","3.5.5","3.7.1","3.7.2")
for (.v in ina) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Inappropriateness")

# Green = Environment
env<- c("2.6.1","2.6.2","2.6.3","2.6.4","2.6.5","2.6.7","2.6.8","2.6.9","2.6.10","2.6.11","2.6.12","2.6.13","2.6.14")
for (.v in env) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Environment")


env_simple <- c("Environment","Environment0","Environment1", "Environment2", "Environment3", "Environment4" )
for (.v in env_simple) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Environmental")

# Gray = Other
gra <- c("2.7.2","2.7.3","3.7.3","3.7.4","3.9.1","3.9.2","3.9.3")
for (.v in gra) pls.datatable$CrimeType <- str_replace(pls.datatable$CrimeType, .v, "Other")
...
for (.v in bijlmer_centrum) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Bijlmer-Centrum")
...
for (.v in bos_en_lommer) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Bos en Lommer")
...
for (.v in buitenveldert_zuidas) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Buitenveldert - Zuidas")
...
for (.v in centrum_oost) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Centrum-Oost")
...
for (.v in centrum_west) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Centrum-West")
...
for (.v in de_akker_nieuw_sloten) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "De Akker - Nieuw Sloten")
...
for (.v in de_pijp_rivierenbuurt) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "De Pijp - Rivierenbuurt")
...
for (.v in gaasperdam_driemond) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Gaasperdam - Driemond")
...
for (.v in geuzenveld_slotermeer) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Geuzenveld - Slotermeer")
...
for (.v in ijburg_zeeburgereiland) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "IJburg - Zeeburgereiland")
...
for (.v in noord_oost) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Noord-Oost")
...
for (.v in noord_west) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Noord-West")
...
for (.v in oostelijk_havengebied_indische_buurt) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Oostelijk Havengebied - Indische Buurt")
...
for (.v in osdorp) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Osdorp")
...
for (.v in de_baarsjes_oud_west) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "De Baarsjes - Oud-West")
...
for (.v in oud_noord) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Oud-Noord")
...
for (.v in oud_oost) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Oud-Oost")
...
for (.v in zuid) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Zuid")
...
for (.v in slotervaart) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Slotervaart")
...
for (.v in watergraafsmeer) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Watergraafsmeer")
...
for (.v in westerpark) pls.datatable$Neighborhoods <- str_replace(pls.datatable$Neighborhoods, .v, "Westerpark")
...
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM01", "Jan2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM02", "Feb2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM03", "Mrt2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM04", "Apr2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM05", "May2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM06", "June2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM07", "July2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM08", "Aug2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM09", "Sept2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM10", "Oct2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM11", "Nov2021")
pls.datatable$Periods <- str_replace(pls.datatable$Periods, "2021MM12", "Dec2021")
...
View(pls.datatable_crime_summarized)
